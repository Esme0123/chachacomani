...
export default function Sidebar({
  ...
  const getChapterIcon = (num) => {
    if (num <= 3) return <Scale className={`w-4 h-4 ${tema.iconoCapitulo}`} />;
  ...
  const etiquetaCapitulo = (cap) => {
    if (strings?.capituloEtiqueta) return strings.capituloEtiqueta(cap);
    return cap.numero_romano === 'D.F.' ? 'DISP. FINALES' : `CAPÍTULO ${cap.numero_romano}`;
  };
    ...
    <div className="h-full flex flex-col bg-cream-50 dark:bg-navy-800 border-r border-sand-300 dark:border-slate-700/60 w-80 select-none">
      ...
      <div className="p-4 border-b border-sand-300 dark:border-slate-800/80 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <BookMarked className={`w-5 h-5 ${tema.indiceIcono}`} />
      ...
      <div className="p-3 border-b border-sand-300 dark:border-slate-800/60 bg-cream-100 dark:bg-navy-950/60">
        <div className={`grid p-1 bg-cream-300/70 dark:bg-navy-800/80 rounded-xl ${tieneAnexos ? 'grid-cols-2' : 'grid-cols-1'} gap-1.5`}>
          <button
            onClick={() => {
          ...
          >
            {strings?.sidebarCapitulos || '21 Capítulos'}
          </button>
          {tieneAnexos && strings?.sidebarAnexos && (
            <button
              onClick={() => {
        ...
        {selectedTab === 'capitulos' || !tieneAnexos ? (
          <>
            <div className="px-2 py-1 text-[11px] font-semibold text-ink-muted dark:text-slate-400 uppercase tracking-wider">
              {strings?.sidebarSubheader || 'Estructura Normativa (Arts. 1 - 105)'}
            </div>
            {capitulos.map((cap) => {
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl transition-all flex items-start gap-2.5 ${
                    isSelected
                      ? `${tema.capituloSeleccionado} font-semibold shadow-sm`
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between text-[11px] text-ink-muted dark:text-slate-400 font-mono">
                      <span>{etiquetaCapitulo(cap)}</span>
                    ...
                    <p className={`text-xs line-clamp-2 mt-0.5 ${isSelected ? tema.capituloTituloSel : 'text-slate-600 dark:text-slate-200'}`}>
                      {cap.titulo}
                    </p>
                  </div>
                </motion.button>
      ...
      <div className="p-3 border-t border-sand-300 dark:border-slate-800/80 text-[11px] text-ink-muted dark:text-slate-400 bg-cream-100 dark:bg-navy-950">
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-1"><Gem className={`w-3 h-3 ${tema.puntoEstado}`} /> {strings?.sidebarFooterIzq || 'Aprobación 2026'}</span>
          <span className={`font-semibold ${tema.footerAcento}`}>{strings?.sidebarFooterDer || 'AFCOOP / FECOMAN'}</span>
        </div>
      </div>
