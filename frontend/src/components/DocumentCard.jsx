...
export default function DocumentCard({ card, dark, onNavigate }) {
  ...
  const contenido = (
    <div
      className="relative group h-full cursor-pointer card-lift"
      onClick={abrir}
    ...
    >
      <div className="relative h-full transition-transform duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
        ...
        {/* ───────── FRENTE ───────── */}
        <div className="absolute inset-0 [backface-visibility:hidden] flex flex-col items-center gap-3 p-5 overflow-hidden">
          {/* Icon ring */}
          <div
        ...
        {/* ───────── REVERSO ───────── */}
        <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] overflow-hidden rounded-[12px] bg-gradient-to-br from-[#0D0B61] via-[#294669] to-[#080640]">
          {card.img && (
            <img
              src={card.img}
              alt={card.title}
          ...
          <div className="relative z-10 h-full flex flex-col items-center justify-center gap-3 p-6 text-center">
            <span className="text-4xl drop-shadow-lg">{card.icon}</span>
            <span className="font-mono text-[10px] tracking-widest text-[#E4D329] border border-[#E4D329]/50 rounded-full px-3 py-1">
              DOCUMENTO OFICIAL
            </span>
            ...
            <button
              onClick={abrir}
              className="mt-2 px-5 py-2 rounded-lg font-display font-bold text-xs tracking-wider text-[#0D0B61] transition-all hover:scale-105"
